function [ci_ub,ci_yvalues_ub,ci_ub_check] = ci_right_endpoint_frac_error(data,C,Ntot,alpha,T,mle,scale,optlikelihood,Aineq,bineq,Aeq,beq,lb,ub,i,j,quant,tol)
...
ci_yvalues_ub = neg_log_likelihood_frac_error(sol,scale_new,C,Ntot,alpha,data,T);
...
function [c,ceq] = constraints_frac(param,scale,C,Ntot,alpha,data,T,optvalue,quant)
    c = neg_log_likelihood_frac_error(param,scale,C,Ntot,alpha,data,T) - optvalue - quant;
    ceq = 0;
end