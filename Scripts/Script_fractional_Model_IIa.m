...
% The script implements estimation under Model IIa of Section "Application:
...
% the model parameters under Model IIa.
...
% of cells of ceah type under the i-th initial condition.
alpha = [1,0;0,1];
...
beq_frac = 0;
% In this script, the relative net birth rate lambda_2-lambda_1 is
% constrained to be zero. To impose the constraint, the coefficient of
% lambda_2-lambda_1 in Aeq_frac is set to 1, and beq_frac is set to 0.
...
[params_mle_frac, error_mle_frac, scale_mle_frac, scale_error_frac, optlikelihood_frac, feasible_frac] ...
 = estimation_mle_frac_error_simple(data_frac,C_frac,Ntot,alpha,T,x0_frac,scale_frac,Aineq_frac,bineq_frac,Aeq_frac,beq_frac,lb_frac,ub_frac,tol);
...
if ~isempty(x0_frac_def)
    for n = 1:size(x0_frac_def,3)
        ...
        [params_mle_temp, error_mle_temp, scale_temp, scale_error_temp, optvalue_temp, feasible_temp]  ...
            = estimation_mle_frac_error_simple(data_frac,C_frac,Ntot,alpha,T,x0_frac_def(:,:,n),scale_frac_def,Aineq_frac,bineq_frac,Aeq_frac,beq_frac,lb_frac,ub_frac,tol);
        ...
        if ci_option(i,j) == 1
            ...
            for n=1:nopt_ci+1
                [ci_frac_left_temp, ci_yvalues_frac_left_temp, ci_check_frac_left_temp] ...
                    = ci_left_endpoint_frac_error_simple(data_frac,C_frac,Ntot,alpha,T,params_mle_frac,scale_mle_frac,optlikelihood_frac,Aineq_frac,bineq_frac,Aeq_frac,beq_frac,lb_frac,ub_frac,i,j,quant,tol);
                ...
                [ci_frac_right_temp, ci_yvalues_frac_right_temp, ci_check_frac_right_temp] ...
                    = ci_right_endpoint_frac_error_simple(data_frac,C_frac,Ntot,alpha,T,params_mle_frac,scale_mle_frac,optlikelihood_frac,Aineq_frac,bineq_frac,Aeq_frac,beq_frac,lb_frac,ub_frac,i,j,quant,tol);
